...
@JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    init {
        context.theme.obtainStyledAttributes(attrs, R.styleable.RoundSquareProgressView, defStyleAttr, defStyleAttr)
                .apply {
                    try {
                        setRadius(getDimension(R.styleable.RoundSquareProgressView_radius, 0f))
                        setBgColor(getColor(R.styleable.RoundSquareProgressView_bg_color, Color.TRANSPARENT))
                        if (!isInEditMode) {
                            setTextFont(getString(R.styleable.RoundSquareProgressView_font_name)
                                    ?: "")
                        }
                        setText(getString(R.styleable.RoundSquareProgressView_text) ?: "")
                        setTextColor(getColor(R.styleable.RoundSquareProgressView_text_color, Color.WHITE))
                        setTextGravity(getInt(R.styleable.RoundSquareProgressView_text_gravity, Gravity.NO_GRAVITY))
                        setProgressWidth(getDimension(R.styleable.RoundSquareProgressView_progress_width, 12f))
                        setProgressPercent(getFloat(R.styleable.RoundSquareProgressView_progress_percent, 0f))
                        setProgressColor(getColor(R.styleable.RoundSquareProgressView_progress_color, Color.GRAY))
                        setProgressBgToStaticColor(getColor(R.styleable.RoundSquareProgressView_progress_bg_color_static, Color.LTGRAY))
                        setProgressBgToDynamicColor(getColor(R.styleable.RoundSquareProgressView_progress_bg_color_dynamic, Color.RED))
                        setProgressStartAngle(getInt(R.styleable.RoundSquareProgressView_progress_start_angle, 0))
                    } finally {
                        recycle()
                    }
                }
    ...
    private fun createProgressPathSegment(path: Path): Path {
        ...
        if (progressPercent.toInt() in 1..100) {
            ...
            pathMeasure.getSegment(offset, pathLength * progressPercent / 100f + offset, pathSegment, true)
    ...
    private fun drawText(canvas: Canvas?) {
        ...
        val offset = progressPaint.strokeWidth + TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, textPadding, resources.displayMetrics)
        if (!tempRect.isEmpty) {
